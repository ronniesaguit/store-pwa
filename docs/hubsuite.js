(function(global) {
  ...
  NEGOSYO: PLAN_IDS.NEGOSYO_HUB,
  NEGOSYO_HUBS: PLAN_IDS.NEGOSYO_HUB,
  STANDARD: PLAN_IDS.BUSINESS_HUB,
  GROWTH: PLAN_IDS.BUSINESS_HUB,
  PRO: PLAN_IDS.BUSINESS_HUB,
  BUSINESS: PLAN_IDS.BUSINESS_HUB,
  BUSINESS_HUBS: PLAN_IDS.BUSINESS_HUB,
  NEXORA: PLAN_IDS.NEXORA_HUB,
  NEXORA_HUBS: PLAN_IDS.NEXORA_HUB,
...
var MODULE_ALIASES = {
  ...
  var MODULE_CATALOG = {
    quick_sell: { code: 'quick_sell', name: 'Quick Sell', icon: '💰', shortDescription: 'POS checkout, cart, receipts and sales history.', sellAsAddOn: false },
    products: { code: 'products', name: 'Products', icon: '📦', shortDescription: 'Product catalog, categories, barcodes and item setup.', sellAsAddOn: false },
    inventory: { code: 'inventory', name: 'Inventory', icon: '📋', shortDescription: 'Stock levels, restocking, adjustments and movement tracking.', sellAsAddOn: false },
    expenses: { code: 'expenses', name: 'Expenses', icon: '💸', shortDescription: 'Expense logging, categories and operating cost tracking.', sellAsAddOn: false },
  reports: { code: 'reports', name: 'Reports', icon: '📊', shortDescription: 'Sales, profit and printable business reports.', sellAsAddOn: false },
  tax_reports: { code: 'tax_reports', name: 'Tax Reports', icon: '🧾', shortDescription: 'Tax-ready summaries, filing references and compliance exports.' },
  activity_log: { code: 'activity_log', name: 'Activity Log', icon: '📜', shortDescription: 'Operational audit trail, user actions and store event history.' },
  staff_management: { code: 'staff_management', name: 'Staff Management', icon: '👥', shortDescription: 'Staff accounts, role assignment and access control.', sellAsAddOn: false },
    feature_marketplace: { code: 'feature_marketplace', name: 'Hub Add-ons', icon: '🛒', shortDescription: 'Browse and activate optional HubSuite add-ons.', sellAsAddOn: false },
    hardware_profiles: { code: 'hardware_profiles', name: 'Hardware Setup', icon: '🖨️', shortDescription: 'Printer, scanner and hardware profile setup.', sellAsAddOn: false },
    settings: { code: 'settings', name: 'Settings', icon: '⚙️', shortDescription: 'Business, operations and printing settings.', sellAsAddOn: false },
    support: { code: 'support', name: 'Support', icon: '📞', shortDescription: 'Help desk access and support messaging.', sellAsAddOn: false },
    suppliers: { code: 'suppliers', name: 'Suppliers', icon: '🏭', shortDescription: 'Supplier directory, profiles and sourcing information.' },
    purchase_orders: { code: 'purchase_orders', name: 'Purchase Orders', icon: '📋', shortDescription: 'PO creation, approval and stock receiving workflow.' },
    approvals: { code: 'approvals', name: 'Approvals', icon: '✅', shortDescription: 'Approval queues for controlled store actions.' },
    monitors: { code: 'monitors', name: 'Monitors', icon: '📡', shortDescription: 'Health, KPI and operational monitoring dashboards.' },
    internal_chat: { code: 'internal_chat', name: 'Internal Chat', icon: '💬', shortDescription: 'Store-to-admin and internal messaging tools.' },
    sandbox_mode: { code: 'sandbox_mode', name: 'Sandbox Mode', icon: '🧪', shortDescription: 'Safe demo and training environment with sample data.' },
    roi: { code: 'roi', name: 'ROI Monitor', icon: '📈', shortDescription: 'Capital, returns and investment health monitoring.' },
    branch_transfer: { code: 'branch_transfer', name: 'Branch Transfers', icon: '🔄', shortDescription: 'Inter-branch stock transfer workflow and receiving.' },
    hq_control_center: { code: 'hq_control_center', name: 'HQ Control', icon: '🏢', shortDescription: 'Multi-branch command center and consolidated views.' },
    custom_role_builder: { code: 'custom_role_builder', name: 'Custom Roles', icon: '🎭', shortDescription: 'Custom role templates and granular permission design.' },
    automation_rules: { code: 'automation_rules', name: 'Automation', icon: '⚡', shortDescription: 'Rule-based automation for recurring business actions.' },
    data_import_tools: { code: 'data_import_tools', name: 'Import & Migration', icon: '📥', shortDescription: 'Bulk import, migration and onboarding data tools.' }
  };

  var TIERS = {
    TRIAL: { id: PLAN_IDS.TRIAL, name: 'HubSuite Trial', shortName: 'Trial', basePrice: 0, addOnPrice: null, logoPath: './assets/branding/hubsuite-trial.svg' },
    NEGOSYO_HUB: { id: PLAN_IDS.NEGOSYO_HUB, name: 'Negosyo Hub', shortName: 'Negosyo', umbrella: 'HubSuite', audience: 'Sari-sari stores and small shops', basePrice: 200, addOnPrice: 30, logoPath: './assets/branding/negosyo-hub.svg' },
    BUSINESS_HUB: { id: PLAN_IDS.BUSINESS_HUB, name: 'Business Hub', shortName: 'Business', umbrella: 'HubSuite', audience: 'Growing establishments and staffed stores', basePrice: 500, addOnPrice: 50, logoPath: './assets/branding/business-hub.svg' },
    NEXORA_HUB: { id: PLAN_IDS.NEXORA_HUB, name: 'Nexora Hub', shortName: 'Nexora', umbrella: 'HubSuite', audience: 'Large retail and multi-branch operations', basePrice: 1000, addOnPrice: 100, logoPath: './assets/branding/nexora-hub.svg' },
    CUSTOM: { id: PLAN_IDS.CUSTOM, name: 'Custom HubSuite Plan', shortName: 'Custom', basePrice: 0, addOnPrice: null, logoPath: './assets/branding/hubsuite.svg' }
  };

  var PLAN_CORE_MODULES = {};
  PLAN_CORE_MODULES[PLAN_IDS.TRIAL] = ['quick_sell','products','inventory','expenses','reports','staff_management','feature_marketplace','hardware_profiles','settings','support'];
  ...
  function getModuleMeta(moduleId) {
    ...
    if (meta) return meta;
    return { code: code, name: _titleizeModule(code), icon: '🧩', shortDescription: '', sellAsAddOn: true };
  }
...
function getAddOnCatalog(planId, featureCatalog) {
  ...
  (Array.isArray(featureCatalog) ? featureCatalog : []).forEach(function(feature) {
    ...
    if (featureMap[code].display_monthly_price == null && addOnPrice !== null) featureMap[code].display_monthly_price = addOnPrice;
    if (!featureMap[code].display_price_note && addOnPrice !== null) featureMap[code].display_price_note = '₱' + addOnPrice + '/month after trial';
    if (!featureMap[code].tenant_status) featureMap[code].tenant_status = 'locked';
    if (!featureMap[code].action_state && featureMap[code].tenant_status === 'locked') featureMap[code].action_state = 'start_trial';
